...
export namespace common {
    ...
    export class RangeInclusiveList extends pb_1.Message {
        ...
        static fromObject(data: {
            ...
            if (data.elements != null) {
                message.elements = data.elements.map((item) =>
                    RangeInclusive.fromObject(item)
                );
            }
            return message;
        }
        toObject() {
            const data: {
                elements?: ReturnType<
                    typeof RangeInclusive.prototype.toObject
                >[];
            } = {};
            if (this.elements != null) {
                data.elements = this.elements.map((item: RangeInclusive) =>
                    item.toObject()
                );
            }
            return data;
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            ...
            if (this.elements.length)
                writer.writeRepeatedMessage(
                    1,
                    this.elements,
                    (item: RangeInclusive) => item.serialize(writer)
                );
            if (!w) return writer.getResultBuffer();
        }
        static deserialize(
